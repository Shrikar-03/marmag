...
export class EmployeeNotificationService {
      ...
      switchMap(userEmail => {
        ...
        return this.firestore.collection<Notification>('employee-notifications', ref =>
          ref.where('userId', '==', userEmail)
            .orderBy('timestamp', 'desc') // Ensure notifications are ordered by timestamp
  ...
  createNotifications(): void {
    ...
    this.authService.getUserEmail().pipe(
      switchMap(userEmail => {
        ...
        // Clear existing notifications for the user at the start of the day
        this.clearNotificationsForEmployee(userEmail);
  ...
  private clearNotificationsForEmployee(userEmail: string): void {
    console.log('Clearing notifications for employee...');
    this.firestore.collection<Notification>('employee-notifications', ref =>
      ref.where('userId', '==', userEmail)
    ).get().pipe(
      switchMap(snapshot => {
        const batch = this.firestore.firestore.batch();
